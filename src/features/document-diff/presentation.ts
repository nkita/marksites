...
.document-diff-toggle[aria-pressed="true"]{color:var(--fgColor-accent,#0969da);background:var(--bgColor-accent-muted,#ddf4ff)}.document-diff-toggle:disabled{color:var(--fgColor-muted,#59636e);opacity:.45;cursor:default}.document-diff-toggle svg[hidden]{display:none}
`;

export const diffScript = `<script>(()=>{
const root=document.querySelector('.document-diff-content');if(!root)return;
const rows=Array.from(root.querySelectorAll('.document-diff-row'));
let pending=false;
