...
- `renderModifiedAtScript()`: `time`要素のISO日時をブラウザのローカルタイムゾーンへ変換し、`Updated YYYY-MM-DD HH:mm:ss TZ`形式で表示する。JavaScript実行前や無効時はUTC表示を維持する。
...
- `createAnnotationsFeature(data?)`: 現在文書のコメントデータ、`Copy`、`Copy for AI`、＋アイコン付き`Comment`を横並びにした選択操作ツールバー、サイドバーへ渡すコメントパネル、件数、スタイル、ブラウザ動作を一体で返す。

ブラウザ側の主要処理:

- `selectionData()`: 本文内の選択だけを受け付け、exact、prefix、suffix、headingId、offsetを作る。
- `copy()`: 選択文字列、AI用Markdown、コメント情報をコピーし、`file://`向けフォールバックを持つ。
- `annotationMetadata()` / `allCommentsMetadata()`: 文書パス、直前の章見出し名、引用、コメント本文だけを含む可読テキストを、単一コメントまたは新しい順の全コメントについて組み立てる。HTML依存の見出しIDと位置、コメント属性の状態、画面表示用の作成・更新日時はコピーへ含めない。古いコメントに見出しIDがない場合は、再特定した引用位置から直前の見出しを補完する。
...
一覧の先頭に文書全体へコメントする追加ボタンを置き、範囲選択コメントと同じフォーム・APIを利用する。追加ボタンの隣には全コメント用のCopyを置き、各カードにも単一コメント用のCopyを置く。Copyは編集可否にかかわらず利用できる。新規追加時のフォームは追加ボタン直下かつコメント一覧の直前へ移動し、編集時だけ対象カード内へ移動する。範囲のないコメントは引用の代わりに`Document comment`と表示し、位置不明として扱わない。一覧には埋め込まれた現在ページのコメントだけを`createdAt`降順で表示し、更新日時はブラウザのローカルタイムゾーンへ変換する。新規コメントの保存後は追加されたカードを先頭に置き、キーボードフォーカスと選択状態を移して表示領域へスクロールする。既存コメントの編集では並び順を変えない。サイドバー内ではコメント本文を13px、引用文を12px、更新日時を11px相当のコンパクトな文字サイズで表示し、引用全文をDOMに保ったままCSSで2行に制限する。コメントカードは主要パネルと同じ8px、コメント欄内の操作ボタンは追加ボタンと同じ6pxの角丸と共通配色を使用する。Copy、Edit、Deleteはカード右上へ重ねる絶対配置とし、非表示時にカード内の領域を確保しない。カードのhover中、操作領域のキーボードフォーカス中だけ表示し、カードの選択状態では表示を維持しない。端末のhover media queryはタッチ対応PCや仮想環境で誤判定されるため使用せず、実際のtouch pointerでカードを選択した場合だけ操作を表示する。フォーカス中のカードと対応する本文ハイライトはアクセント背景・境界色へ変え、通常表示やhoverと区別する。`file://`では追加ボタンを無効にしてAPIへ接続せず閲覧専用とするが、CopyはClipboard APIの代替処理で使用できる。選択操作は目次、ファイルツリー、コード操作、コメントUI自身を対象外にする。コメント本文はDOMの`textContent`で表示する。

Edit時は共通フォームを対象カード内へ移動し、引用を対象箇所として残したまま本文・操作ボタンをフォームへ置き換えてその場で編集する。編集中は選択カードのアクセント背景と太いoutlineを解除し、通常背景、境界線、控えめなshadowでフォーム状態を示す。textareaは6pxの角丸とfocus ringを持ち、操作行は右寄せしてSaveをprimary、Cancelをsecondaryとして表示する。保存またはキャンセル後は通常のカード表示へ戻す。Copy、Edit、Deleteは文字ラベルを持たない28pxのアイコン操作として一体型ツールバーへまとめ、用途は`aria-label`と`title`で示す。削除操作はhover時だけdanger色で区別する。
