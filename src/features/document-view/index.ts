...
export function createDocumentViewFeature(
  ...
  const control = `<button type="button" class="document-content-action document-preview-toggle" data-document-preview-toggle aria-label="プレビューを表示" title="プレビューを表示" aria-pressed="true"><span>プレビュー</span></button>
<button type="button" class="document-content-action document-source-toggle" data-document-source-toggle aria-label="コードを表示" title="コードを表示" aria-pressed="false"><span>コード</span></button><span class="document-view-shortcut-hint"><span>右へ移動：<kbd>L</kbd></span><span>左へ移動：<kbd>H</kbd></span></span>`;
...
@media(hover:hover){.markdown-source-line:hover{background:linear-gradient(to right,color-mix(in srgb,var(--bgColor-accent-muted,#ddf4ff) 35%,var(--bgColor-default,#fff)) 0 47px,var(--borderColor-muted,#d8dee4) 47px 48px,color-mix(in srgb,var(--bgColor-accent-muted,#ddf4ff) 35%,var(--bgColor-default,#fff)) 48px)}.markdown-source-line:hover::before{background:color-mix(in srgb,var(--bgColor-accent-muted,#ddf4ff) 35%,var(--bgColor-default,#fff))}}@media(hover:hover) and (max-width:600px){.markdown-source-line:hover{background:linear-gradient(to right,color-mix(in srgb,var(--bgColor-accent-muted,#ddf4ff) 35%,var(--bgColor-default,#fff)) 0 39px,var(--borderColor-muted,#d8dee4) 39px 40px,color-mix(in srgb,var(--bgColor-accent-muted,#ddf4ff) 35%,var(--bgColor-default,#fff)) 40px)}}
`;
  const script = `<script>(()=>{
const parameter='document-view',previewButton=document.querySelector('[data-document-preview-toggle]'),sourceButton=document.querySelector('[data-document-source-toggle]'),diffButton=document.querySelector('[data-document-diff-toggle]'),current=document.querySelector('.markdown-content'),source=document.querySelector('.markdown-source-content'),diff=document.querySelector('.document-diff-content'),available=${hasDiff},pageUrl=new URL(location.href);
...
function syncLabels(){const english=document.body.dataset.language==='en',showDiff=document.body.dataset.documentView==='diff',previewLabel=english?'Show preview':'プレビューを表示',sourceLabel=english?'Show code':'コードを表示';previewButton.setAttribute('aria-label',previewLabel);previewButton.title=previewLabel;sourceButton.setAttribute('aria-label',sourceLabel);sourceButton.title=sourceLabel;if(diffButton){const diffLabel=available?(showDiff?(english?'Show latest':'最新版を表示'):(english?'Show changes':'差分を表示')):(english?'No changes since the previous build':'前回からの変更はありません');diffButton.setAttribute('aria-label',diffLabel);diffButton.title=diffLabel}}
function apply(view,write=true){intent=view;const actual=view==='markdown'?'markdown':view==='diff'&&available?'diff':'current',showCurrent=actual==='current',showSource=actual==='markdown',showDiff=actual==='diff';document.body.dataset.documentView=actual;current.hidden=!showCurrent;source.hidden=!showSource;if(diff)diff.hidden=!showDiff;previewButton.setAttribute('aria-pressed',String(showCurrent));sourceButton.setAttribute('aria-pressed',String(showSource));if(diffButton)diffButton.setAttribute('aria-pressed',String(showDiff));syncLabels();if(write)history.replaceState(null,'',update(new URL(location.href)));syncLinks()}
window.marksitesSyncDocumentViewLabels=syncLabels;
previewButton.addEventListener('click',()=>apply('current'));sourceButton.addEventListener('click',()=>apply('markdown'));if(available&&diffButton)diffButton.addEventListener('click',()=>apply(document.body.dataset.documentView==='diff'?'current':'diff'));addEventListener('keydown',event=>{if(event.repeat||event.shiftKey||event.ctrlKey||event.metaKey||event.altKey||event.target.closest('input,textarea,select,[contenteditable]:not([contenteditable="false"])'))return;const key=event.key.toLowerCase();if(key!=='h'&&key!=='l')return;const views=available?['current','markdown','diff']:['current','markdown'],currentIndex=views.indexOf(document.body.dataset.documentView),view=views[currentIndex+(key==='l'?1:-1)];if(!view)return;event.preventDefault();apply(view)});apply(intent,available||intent!=='diff');
})()</script>`;
