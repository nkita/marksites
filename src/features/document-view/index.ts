...
export function createDocumentViewFeature(
  ...
  const control = `<button type="button" class="document-content-action document-preview-toggle" data-document-preview-toggle aria-label="プレビューを表示" title="プレビューを表示" aria-pressed="true"><span>プレビュー</span><kbd aria-hidden="true">1</kbd></button>
<button type="button" class="document-content-action document-source-toggle" data-document-source-toggle aria-label="コードを表示" title="コードを表示" aria-pressed="false"><span>コード</span><kbd aria-hidden="true">2</kbd></button>`;
...
previewButton.addEventListener('click',()=>apply('current'));sourceButton.addEventListener('click',()=>apply('markdown'));if(available&&diffButton)diffButton.addEventListener('click',()=>apply(document.body.dataset.documentView==='diff'?'current':'diff'));addEventListener('keydown',event=>{if(event.repeat||event.ctrlKey||event.metaKey||event.altKey||event.target.closest('input,textarea,select,[contenteditable]:not([contenteditable="false"])'))return;const view={'1':'current','2':'markdown','3':'diff'}[event.key];if(!view||view==='diff'&&!available)return;event.preventDefault();apply(view)});apply(intent,available||intent!=='diff');
})()</script>`;
