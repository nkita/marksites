...
treeHashはMarkdown相対パスと各文書のコメント件数から計算する。ファイル構成またはコメント件数が変わった場合は、全ページに埋め込まれたファイルツリーを更新するため全HTMLを再生成する。コメント本文の編集など件数を変えないメタデータ変更は対象HTMLだけを生成する。renderFingerprintが変わった場合も全HTMLを再生成し、manifestは全変換成功後にだけ更新する。
