...
右サイドバーは「目次」と「コメント」のARIA tabで共有する。デスクトップでは内容に応じた高さとし、画面内の最大高を超える場合だけ、タブヘッダーを固定したまま選択パネルをサイドバー内で独立スクロールさせる。コメントのアーカイブを閉じた場合は、非表示になった内容の高さを残さない。これにより、目次が長い場合でもコメントタブが下方へ押し出されない。900px以下ではサイドバー全体を本文上部のstickyメニューとして折り畳む。

コメント一覧には現在表示している文書の隣接JSONにあるコメントだけを表示し、他文書のコメントは取得・埋め込みしない。タブには現在文書のコメント件数を表示する。一覧先頭の追加ボタンから範囲に紐づかない文書全体コメントも作成できる。既存schemaとの互換性を保つため、空の`exact`、`prefix`、`suffix`、nullの`headingId`、0のoffsetで表現する。コメントカードを選ぶと本文の対象箇所へ移動し、本文ハイライトを選ぶとコメントタブへ切り替えて対応カードをアクセント色で強調する。引用は全文をDOMへ保持し、一覧上はCSSで2行に制限する。コメントカードはページの主要パネルと同じ8px、コメント欄内のボタンは追加ボタンと同じ6pxの角丸へ統一する。EditとDeleteはカード右上へ重ね、非表示時にカードの高さへ影響させない。カードのhover中または操作領域のキーボードフォーカス中だけ表示し、カード選択後も表示を固定しない。タッチ対応PCや仮想環境での誤判定を避けるため、hover media queryによる常時表示は行わない。コメント作成・編集後もコメントタブを維持する。コメントが0件の場合は空状態を表示する。

Edit時は対象カードの引用を残し、本文を編集フォームへ置き換える。編集中は選択状態のアクセント背景と太いoutlineを解除し、入力欄へfocus ringを表示する。操作行は右寄せし、Saveをprimary、Cancelをsecondaryとして区別する。保存・キャンセル後にカード表示へ戻す。EditとDeleteは28pxのアイコン操作として一体型の小さなツールバーへまとめ、削除操作はhover時にdanger色で区別する。
...
Markdownの追加・削除・移動、およびコメント件数の変更で全HTMLを更新する理由は、現在のファイルツリーとコメント件数バッジが各HTMLへ埋め込まれているためである。本文だけの更新や件数を変えないコメント編集ではツリー構造が変わらないため、対象HTMLだけを変換できる。バッジは1件以上の場合だけ表示し、ファイル名フィルターの検索文字列には含めない。ディレクトリの合計バッジは閉じている場合だけ表示し、開いた場合は子要素の件数と重複するため非表示にする。
