...
`serve`のポートを省略した場合は3000から開始し、`EADDRINUSE`のときだけ次の番号を昇順に探索する。`--port`を明示した場合は自動変更せず、競合をエラーにする。`--port 0`はOSによる空きポート割り当てとして扱う。起動ログと`--open`には、実際に待ち受けたポートを使用する。
...
右サイドバーは「目次」と「コメント」のARIA tabで共有する。デスクトップではタブヘッダーを固定し、選択パネルだけをサイドバー内で独立スクロールさせる。これにより、目次が長い場合でもコメントタブが下方へ押し出されない。900px以下ではサイドバー全体を本文上部のstickyメニューとして折り畳む。

コメント一覧には現在表示している文書の隣接JSONにあるコメントだけを表示し、他文書のコメントは取得・埋め込みしない。タブには現在文書のコメント件数を表示する。一覧先頭の追加ボタンから範囲に紐づかない文書全体コメントも作成できる。既存schemaとの互換性を保つため、空の`exact`、`prefix`、`suffix`、nullの`headingId`、0のoffsetで表現する。コメントカードを選ぶと本文の対象箇所へ移動し、本文ハイライトを選ぶとコメントタブへ切り替えて対応カードをアクセント色で強調する。引用は全文をDOMへ保持し、一覧上はCSSで2行に制限する。コメントカードはページの主要パネルと同じ8px、コメント欄内のボタンは追加ボタンと同じ6pxの角丸へ統一する。EditとDeleteはカードのhover中またはボタン自身のキーボードフォーカス中だけ表示し、カード選択後も表示を固定しない。タッチ対応PCや仮想環境での誤判定を避けるため、hover media queryによる常時表示は行わない。コメント作成・編集後もコメントタブを維持する。コメントが0件の場合は空状態を表示する。

`file://`でも同じ一覧を閲覧できるが、作成・編集・削除は無効のままとする。ローカルサーバー接続時だけ既存runtime capability確認後に編集操作を有効化する。
...
| コメントJSONの変更（件数が同じ）            | 対応HTMLのみ。静的HTMLへ最新コメントを埋め込む |
| コメント件数の変更                          | 全HTML。ファイルツリーの件数バッジを更新する   |
...
Markdownの追加・削除・移動、およびコメント件数の変更で全HTMLを更新する理由は、現在のファイルツリーとコメント件数バッジが各HTMLへ埋め込まれているためである。本文だけの更新や件数を変えないコメント編集ではツリー構造が変わらないため、対象HTMLだけを変換できる。バッジは1件以上の場合だけ表示し、ファイル名フィルターの検索文字列には含めない。
