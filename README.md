...
Markdownの表では、列境界をドラッグして列幅を変更できます。見出しをクリックすると、その列を昇順、降順、元の順序へ切り替えられます。長い表ではページスクロール中も見出しを固定ヘッダー直下に表示し、横スクロール、列幅、ソート状態へ追従します。列幅変更とソートはマウス、タッチ、キーボードで操作でき、すべて生成HTML内のJavaScriptだけで動作します。
