...
差分ボタンを押すと、左に前バージョン、右に現バージョンを並べて表示します。削除箇所は赤、追加箇所は青で強調し、差分を示す取り消し線は使いません。本文は12px、コードは11pxのコンパクトな表示です。

対応する段落の開始位置を揃え、片側だけの追加・削除には反対側に空白を設けます。コード行、単純なリスト項目、表の行も高さを揃えるため、長い文章が折り返されても次の内容を左右で追えます。狭い画面では2列の比較領域を横スクロールできます。
...
左ファイルサイドバーは幅280pxとし、表示状態はLayoutメニューから変更します。更新日時はヘッダーのファイルパス右隣へ細い縦線で区切って表示し、狭い画面では省略します。
...
本文上部の`Preview`と`コード`で、変換後の表示と元のMarkdownを切り替えられます。コードビューは行番号とMarkdown構造の装飾を表示し、カーソルを重ねた行を淡くハイライトします。選択した表示はURLへ反映され、サイト内のHTMLページ間でも引き継がれます。
