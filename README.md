...
本文上部の`プレビュー`、`コード`の右にある差分ボタンを押すと、左に前バージョン、右に現バージョンを並べて表示します。削除箇所は赤、追加箇所は青で強調し、差分を示す取り消し線は使いません。本文は12px、コードは11pxのコンパクトな表示です。
...
asdfasdfasd
f
時を表示する場合は、ISO 81形式で指定します。CLI変換ではMarkdownファイルの最終更新日時が自動的に使用されます。
...
見出しレベル2から6までの目次と、GitHub互換の見出しアンカーが自動生成されます。ファイルツリーは画面左側、`目次`とコメントは画面右側へ配置し、本文を中央に表示します。ファイル一覧は階層ツリーと更新順を切り替えられ、更新順ではMarkdownのローカル更新日時を新しい順に日付でまとめます。親ディレクトリはファイルへのホバーまたはフォーカス時に吹き出しで表示します。サイドバーの閉状態、ポップオーバー、更新順表示、開いたフォルダはGETパラメータでHTMLページ間に引き継がれます。ファイルパス直後のアイコンでパスをコピーでき、更新日時はブラウザのローカルタイムゾーンで表示します。現在位置に対応する見出しもハイライトされます。

ヘッダー右端の`Layout`メニューから「すべて表示」「左＋本文」「本文＋右」「本文のみ」を選択し、選択後のレイアウトを直接指定できます。左端の単独開閉ボタンと右サイドバーの単独ボタンは表示しません。閉状態は`file-sidebar=closed`と`document-sidebar=closed`としてHTMLページ間に引き継ぎます。
asdfasdf
```ts
const html = markdownToHtml(markdown, {
  tableOfContents: {
...
本文上部の`プレビュー`と`コード`で、変換後の表示と元のMarkdownを切り替えられます。コードビューは行番号とMarkdown構造の装飾を表示し、カーソルを重ねた行を淡くハイライトします。選択した表示はURLへ反映され、サイト内のHTMLページ間でも引き継がれます。

言語名を指定したコードブロックは、`highlight.js` によって自動的にハイライトされます。
コードブロックには、コードをクリップボードへコピーするボタンと、長い行の折り返しを切り替えるボタンも表示されます。

Markdownの表では、右上の「コピー」リンクで見出しを含む表全体を現在の並び順でコピーできます。カンマ、引用符、セル内改行に対応し、`file://`でも利用できます。列境界をドラッグして列幅を変更できます。見出しをクリックすると、その列を昇順、降順、元の順序へ切り替えられます。長い表ではページスクロール中も見出しを固定ヘッダー直下に表示し、横スクロール、列幅、ソート状態へ追従します。列幅変更とソートはマウス、タッチ、キーボードで操作でき、すべて生成HTML内のJavaScriptだけで動作します。
...
本文で文字列を選択すると、コンテキストメニューのコピー操作の隣に`文字列置換`が表示されます。選択文字列を変換元として画面中央の管理ポップアップへ置換後の文字列を入力すると、表示中の本文にある同じ文字列をすべてブラウザ上で一時的に置換でき、アクセストークンなどを当てはめてからコピーできます。置換はこのコンテキストメニューからだけ開始でき、複数ルールを一覧で管理して個別にクリアできます。ポップアップは閉じるアイコンまたは範囲外のクリックで閉じます。置換箇所は枠付きでハイライトされます。元のMarkdownと生成HTMLは変更されず、ページを再読み込みしても元に戻ります。
